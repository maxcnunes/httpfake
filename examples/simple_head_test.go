...
// TestSimpleHead tests a fake server handling a POST request
func TestSimpleHead(t *testing.T) {
	...
	req, err := http.NewRequest("HEAD", fmt.Sprintf("%s/users", fakeService.Server.URL), nil)
	if err != nil {
		t.Fatal(err)
	}
